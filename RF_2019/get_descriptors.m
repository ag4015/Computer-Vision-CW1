...
for c = 1:length(classList)
    ...
    for k = 1:size(imgList(1).name,2)
        imgList_name(k,:) = imgList(k).name;
    end
    imgIdx{c} = randperm(length(imgList));
    imgIdx_tr = imgIdx{c}(1:imgSel(1));
    csvwrite(strcat('imgIdx/', classes{c}, '_tr_idx.csv'), imgIdx_tr);
    imgIdx_te = imgIdx{c}(imgSel(1)+1:sum(imgSel));
    csvwrite(strcat('imgIdx/', classes{c}, '_te_idx.csv'), imgIdx_te);
